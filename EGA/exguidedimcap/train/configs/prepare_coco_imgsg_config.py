...
def prepare_attention():
    ...
    mp_ft_dir = os.path.join(ROOT_DIR, 'features', 'MP')
    attn_ft_dir = os.path.join(ROOT_DIR, 'features', 'SA')
    split_dir = os.path.join(anno_dir, 'public_split')
    res_dir = os.path.join(ROOT_DIR, 'results', 'EGuidedCAP')
    ...
    output_dir = os.path.join(res_dir,
                              ...
                              (mp_ft_name, attn_ft_name,
                               'rgcn.%d.' % attn_enc_cfg.num_hidden_layers,
                               dec_cfg.rnn_type,
    ...
    output_dir = '%s%s' % (output_dir, '.embed_first' if attn_enc_cfg.embed_first else '')
    ...
    for setname in ['trn', 'val', 'val_sub', 'val_sub_opt']:
